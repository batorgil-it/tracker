...
tz_AU = pytz.timezone('Asia/Ulaanbaatar') 
...
def draw_boxes(img, bbox, identities=None, offset=(0, 0)):
    for i, box in enumerate(bbox):
        ...
        if id not in ids and y2 > 480 and y2 < 545:
            ...
            if not os.path.isdir(f'inference/{now.year}/{now.month}/{now.day}'):
                os.makedirs(f'inference/{now.year}/{now.month}/{now.day}')
            
            cv2.imwrite(f'inference/{now.year}/{now.month}/{now.day}/car_{now.strftime("%Y%m%d%H%M%S")}.jpg', crop_img)
            ids.append(id)
